...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        """
          ...
          where the maxValue is over legal actions.  Note that if
    ...
    def update(self, state, action, nextState, reward):
        ...
        for nextAction in legalActions:
            QValue = self.QValue[(nextState, nextAction)]
            if QValue > maxValue:
                maxValue = QValue
        ...
        self.QValue[(state, action)] = self.QValue[(state, action)] + self.alpha * (sample - self.QValue[(state, action)])
